...
def get_coco_annotation_from_obj(obj, label2id):
    ...
    # Map VOC label to COCO label if possible
    if label in voc_to_coco:
        label = voc_to_coco[label]
...
def convert_xmls_to_cocojson(
    ...
    for idx, a_path in enumerate(tqdm(annotation_paths)):
        ...
        img_info = get_image_info(
            annotation_root=ann_root, extract_num_from_imgid=False
        )
        # Assign integer id
        img_id = idx + 1
        img_id_map[img_info["id"]] = img_id
        img_info["id"] = img_id
